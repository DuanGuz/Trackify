...
class RegistroUsuarioRRHHForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name in ("rol", "departamento"):
            self.fields[name].widget.attrs.update({
                "class": "form-control",      
                "data-toggle": "select"        
            })

        # Asignar empresa a instancia nueva
    ...
    def clean(self):
        ...
        # RRHH NO debe tener departamento
    ...
    def save(self, commit=True):
        ...
        # Asegurar que RRHH quede sin departamento
...
class TareaForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # 👉 APLICAR ESTILO A LOS SELECTS (DEPARTAMENTO / ASIGNADO)
        for name in ("departamento", "asignado"):
            if name in self.fields:
                self.fields[name].widget.attrs.update({
                    "class": "form-control",    # si tu tema usa "custom-select", cámbialo aquí
                    "data-toggle": "select"     # Tiny Dashboard lo usa para estilizar selects
                })

        # Asegurar empresa en la instancia
        if self.empresa and not getattr(self.instance, "empresa_id", None):
            self.instance.empresa = self.empresa
        ...
        if u.rol.nombre == 'Gerente':
            if u.departamento_id:
                ...
                self.fields['departamento'].queryset = self.fields['departamento'].queryset.filter(
                    pk=u.departamento_id
                )
                self.fields['asignado'].queryset = User.objects.filter(
                    empresa=self.empresa,
                    rol__nombre='Supervisor',
...
class EvaluacionForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "finalidad": forms.Select(attrs={"class": "form-control"}),  # se complementa en __init__
            # si quieres, también puedes definir comentarios aquí, por ejemplo:
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name in ("evaluado", "finalidad"):
            if name in self.fields:
                self.fields[name].widget.attrs.update({
                    "class": "form-control",   
                    "data-toggle": "select",   
    ...
    def clean(self):
        ...
        # setear evaluador y tipo en la instancia
        self.instance.evaluador = user
        if rol == "Gerente":
            self.instance.tipo = "SUPERVISOR"
        elif rol == "Supervisor":
            self.instance.tipo = "TRABAJADOR"
        else:
            self.instance.tipo = None

        # asegurar empresa en la instancia
        if (
            self.instance
            and hasattr(self.instance, "empresa_id")
            and not self.instance.empresa_id
            and self.empresa
        ):
            self.instance.empresa = self.empresa
